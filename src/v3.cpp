...
void debug_set_level(const int db_level) { debug_level = db_level; }
    ...
    const char* fileName, const OctetStr& engineId, unsigned int& boot)
{
    ...
    if (len > MAXLENGTH_ENGINEID)
    {
        ...
        LOG("getBootCounter: engine id too long, ignoring last bytes (len) "
            "(max)");
        LOG(len);
    ...
    while (fgets(line, MAX_LINE_LEN, file))
    {
        ...
        if (!strncmp(encoded, line, len * 2 + 1))
        {
            ...
            LOG_BEGIN(loggerModuleName, DEBUG_LOG | 3);
            LOG("getBootCounter: found entry (file) (engine id) (boot "
                "counter)");
            LOG(fileName);
    ...
    const char* fileName, const OctetStr& engineId, const unsigned int boot)
{
    ...
    {
        LOG_BEGIN(loggerModuleName, ERROR_LOG | 3);
        LOG("saveBootCounter: engine id too long, ignoring last bytes (len) "
            "(max)");
        LOG(len);
        LOG(MAXLENGTH_ENGINEID);
        LOG_END;
    ...
    if (!file_in)
    {
        ...
        fputs("# \n", file_in);
        fputs(
            "# This file was created by an SNMP++v3 application,\n", file_in);
        fputs(
            "# it is used to store the snmpEngineBoots counters.\n", file_in);
        fputs("# \n", file_in);
    ...
    if ((file_in) && (file_out))
    {
        ...
        while (fgets(line, MAX_LINE_LEN, file_in))
        {
            ...
            if (!strncmp(encoded, line, len * 2 + 1))
            {
                ...
                {
                    LOG_BEGIN(loggerModuleName, WARNING_LOG | 3);
                    LOG("saveBootCounter: Removing doubled entry (file) "
                        "(line)");
                    LOG(fileName);
                    LOG(line);
                    LOG_END;
...
#    endif
        ...
        {
            LOG_BEGIN(loggerModuleName, ERROR_LOG | 1);
            LOG("saveBootCounter: Failed to rename temporary file (tmp file) "
                "(file)");
            LOG(tmpFileName);
            LOG(fileName);
            LOG_END;
