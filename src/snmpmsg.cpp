...
int SnmpMessage::load(
    ...
    v3MP* mpv3,
#endif
    const Pdu& cpdu, const OctetStr& community, const snmp_version version,
    ...
    const int security_model)
{
    ...
    if (version == version3)
    {
        ...
        {
            LOG_BEGIN(loggerModuleName, ERROR_LOG | 4);
            LOG("SNMPMessage: Need security name and engine id for v3 "
                "message");
            LOG_END;
        ...
        status =
            mpv3->snmp_build(raw_pdu, databuff, (int*)&bufflen, *engine_id,
                *security_name, security_model, pdu->get_security_level(),
                pdu->get_context_engine_id(), pdu->get_context_name());
...
int SnmpMessage::load(unsigned char* data, unsigned long len)
{
    ...
    {
        memcpy(
            (unsigned char*)databuff, (unsigned char*)data, (unsigned int)len);
        bufflen    = len;
        valid_flag = true;
    }
    else
    ...
    SmiINT32* security_model, UdpAddress* from_addr, Snmp* snmp_session)
{
    ...
    if ((security_model) && (security_name) && (engine_id) && (snmp_session))
    {
        ...
        status = snmp_session->get_mpv3()->snmp_parse(snmp_session, raw_pdu,
            databuff, (int)bufflen, *engine_id, *security_name,
            context_engine_id, context_name, security_level, *security_model,
            version, *from_addr);
        if (status != SNMPv3_MP_OK)
        {
...
#endif
    {
        ...
        status = snmp_parse(raw_pdu, databuff, (int)bufflen, community_name,
            community_len, version);
        if (status != SNMP_CLASS_SUCCESS)
        {
            snmp_free_pdu(raw_pdu);
            return status;
