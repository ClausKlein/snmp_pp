...
CUDEvent::CUDEvent(const UdId uniqueId, const int fd, const UdInputMask mask,
    const ud_callback callBack, const void* callData)
    : m_uniqueId(uniqueId), m_fd(fd), m_mask(mask), m_callBack(callBack),
...
CUDEventQueue::CUDEventQueueElt::CUDEventQueueElt(
    CUDEvent* udevent, CUDEventQueueElt* next, CUDEventQueueElt* previous)
...
UdId CUDEventQueue::AddEntry(const int fd, const UdInputMask mask,
    const ud_callback callBack, const void* callData)
{
...
void CUDEventQueue::GetFdSets(int& maxfds, fd_set& readfds, fd_set& writefds,
    ...
    while (msgEltPtr)
    {
        ...
        if (mask & UdInputReadMask)
        {
            FD_SET(fd, &readfds);
        }
        if (mask & UdInputWriteMask)
        {
            FD_SET(fd, &writefds);
        }
        if (mask & UdInputExceptMask)
        {
            FD_SET(fd, &exceptfds);
        }
        if (maxfds < (fd + 1))
