    ...
    const int reason, const Pdu& pdu, const UdpAddress& fromaddress)
{
    ...
    if (Pdu::match_type(m_pdu.get_type(), pdu.get_type()) == false)
    {
        ...
        LOG("MsgQueue: Response pdu type does not match, pdu is ignored: (id) "
            "(type1) (type2)");
        LOG(m_uniqueId);
        LOG(m_pdu.get_type());
        LOG(pdu.get_type());
    ...
    if (m_received)
    {
        ...
        LOG("MsgQueue: Message is already marked as received (id) (reason) "
            "(new reason)");
        LOG(m_uniqueId);
        LOG(reason);
        LOG(m_reason);
...
CSNMPMessageQueue::CSNMPMessageQueueElt::CSNMPMessageQueueElt(
    CSNMPMessage* message, CSNMPMessageQueueElt* next,
...
#    ifdef _SNMPv3
            if (engine_id.len() > 0)
            {
                    ...
                    && (target->get_version() == version3))
                {
                    ...
                    m_snmpSession->get_mpv3()->add_to_engine_id_table(
                        engine_id, (char*)addr.IpAddress::get_printable(),
                        addr.get_port());
                }
            }
#    endif
    ...
    const fd_set& /*writefds*/, const fd_set& /*exceptfds*/)
{
    ...
    {
        if ((FD_ISSET(fd, &snmp_readfds)) && (FD_ISSET(fd, (fd_set*)&readfds)))
        {
            UdpAddress fromaddress;
            Pdu        tmppdu;
            int        status      = 0;
