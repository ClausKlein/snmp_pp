                ...
                && (cbData->reports_received <= 1))))
    {
        ...
        if ((cbData->pdu) && (cbData->target))
        {
            rc = snmp->snmp_engine(*(cbData->pdu), cbData->non_reps,
                ...
                (snmp_callback)(cbData->oldCallback), (void*)cbData->cbd,
                INVALID_SOCKET, cbData->reports_received + 1);
...
int receive_snmp_response(SnmpSocket sock, Snmp& snmp_session, Pdu& pdu,
    UdpAddress& fromaddress, OctetStr& engine_id, bool process_msg = true)
{
    ...
    {
        debugprintf(4,
            "receive_snmp_response: engine_id (%s), security_name (%s), "
...
int receive_snmp_notification(
    SnmpSocket sock, Snmp& snmp_session, Pdu& pdu, SnmpTarget** target)
{
    unsigned char    receive_buffer[MAX_SNMP_PACKET + 1];
    long             receive_buffer_len = 0; // len of received data
    SocketAddrType   from_addr;
    ...
    if (version == version3)
    {
        *target = new UTarget();
        ...
        debugprintf(4,
            "receive_snmp_notification: engine_id (%s), security_name "
...
Snmp::Snmp(int& status, const UdpAddress& addr_v4, const UdpAddress& addr_v6)
    : SnmpSynchronized(),
#ifdef _SNMPv3
      mpv3(v3MP::instance),
#endif
...
#    endif
    ...
    if (addresses[0])
    {
        ...
        if ((iv_snmp_session = socket(AF_INET, SOCK_DGRAM, 0))
            == INVALID_SOCKET)
        {
#ifdef WIN32
        ...
        else
        {
                ...
                < 0)
            {
                ...
                {
                    debugprintf(0,
                        "Uncatched errno value %d, returning internal error.",
                        errno);
                    status = SNMP_CLASS_INTERNAL_ERROR;
                }
#endif
...
Snmp::~Snmp()
{
    ...
    if (iv_snmp_session_ipv6 != INVALID_SOCKET)
    {
        // go through the snmpEventList and delete any outstanding
        // events on this socket
        eventListHolder->snmpEventList()->DeleteSocketEntry(
...
const char* Snmp::get_version() { return SNMP_PP_VERSION_STRING; }
...
//------------------------[ get async ]----------------------------------
int Snmp::get(Pdu& pdu, SnmpTarget& target, const snmp_callback callback,
...
//------------------------[ get next async ]-----------------------------
int Snmp::get_next(Pdu& pdu, SnmpTarget& target, const snmp_callback callback,
    const void* callback_data)
{
...
//------------------------[ set async ]----------------------------------
int Snmp::set(Pdu& pdu, SnmpTarget& target, const snmp_callback callback,
    const void* callback_data)
{
    ...
    const void*         callback_data)         // callback data
{
    ...
    return snmp_engine(
        pdu, non_repeaters, max_reps, target, callback, callback_data);
}
...
#else
        return send_snmp_request(iv_snmp_session, send_buf, send_len, address);
#endif
    }
}
    ...
    const SnmpTarget& target) // destination target
{
    ...
    { // target is not a CTarget:
        ...
        { // target is neither CTarget nor UTarget:
    ...
    if (version == version3)
    {
        ...
        {
            debugprintf(0,
                "-- SNMP++, dont know how to handle SNMPv3 without UTarget!");
            return SNMP_CLASS_INVALID_TARGET;
        }
...
void Snmp::check_notify_timestamp(Pdu& pdu)
{
    ...
    if (timestamp <= 0)
    {
        ...
        timestamp = SAFE_ULONG_CAST(
            (timebuffer.time * 100) + (timebuffer.millitm / 10));
...
int Snmp::get_notify_filter(OidCollection& trapids, TargetCollection& targets)
{
    ...
    const void* callback_data)
{
    ...
    return eventListHolder->notifyEventList()->AddEntry(
        this, trapids, targets);
}
...
int Snmp::snmp_engine(Pdu& pdu,      // pdu to use
    ...
    SnmpSocket fd, int reports_received)

{
    ...
    for (int maxloops = 0; maxloops < 3; maxloops++)
    {
        ...
        if (ctarget) /* Is is a CTarget? */
        {
...
#ifdef _SNMPv3
            if ((version == version3) || (action == sNMP_PDU_REPORT))
            {
                debugprintf(0, "-- SNMP++, use UTarget for SNMPv3");
                return SNMP_CLASS_INVALID_TARGET;
            }
            ...
            //]-----------------
            if ((action == sNMP_PDU_GET) || (action == sNMP_PDU_GET_ASYNC)
        ...
        else if (utarget) /* Is is a UTarget? */
        {
            ...
            if (version != version3)
            {
                ...
                {
                    LOG_BEGIN(loggerModuleName, ERROR_LOG | 1);
                    LOG("Snmp: Target does not match SNMP version: (security "
                        "model) (version)");
                    LOG(security_model);
                    LOG(version);
                    LOG_END;
        ...
        if (version == version3)
        {
            ...
            {
                debugprintf(
                    0, "-- SNMP++, need UTarget to send SNMPv3 message!");
                return SNMP_CLASS_INVALID_TARGET;
            }
            OctetStr engine_id;
            ...
            if (engine_id.len() == 0)
            {
                if (mpv3->get_from_engine_id_table(
                        engine_id, udp_address.get_printable())
                    == SNMPv3_MP_OK)
                {
                    // Override const here
                ...
                else
                {
                    ...
                    {
                        // no engine id, discovery disabled and not
                        // authoritytive
                        LOG_BEGIN(loggerModuleName, ERROR_LOG | 1);
                        LOG("Not authoritative and discovery disabled. Target "
                            "without engine id is invalid");
                        LOG_END;
                        return SNMP_CLASS_INVALID_TARGET;
                    }
                }
            ...
            debugprintf(4,
                "Snmp::snmp_engine: engineID (%s), securityName (%s)"
...
#endif
            status =
                snmpmsg.load(pdu, community_string, (snmp_version)version);
            ...
            && (pdu_action != sNMP_PDU_REPORT))
        {
                    ...
                    || (action == sNMP_PDU_INFORM_ASYNC)))
            {
                ...
                eventListHolder->snmpEventList()->AddEntry(req_id, this,
                    iv_session_used, target, pdu, snmpmsg.data(),
                    (size_t)snmpmsg.len(), udp_address, v3CallBack,
                    (void*)v3CallBackData);
            }
            else
#endif
            {
                eventListHolder->snmpEventList()->AddEntry(req_id, this,
                    iv_session_used, target, pdu, snmpmsg.data(),
                    (size_t)snmpmsg.len(), udp_address, cb, (void*)cbd);
            }
        }
    ...
    OctetStr& engine_id, const int timeout_sec, const UdpAddress& addr)
{
    ...
    unsigned char snmpv3_message[60] = {
        ...
        0x30, 0x12, 0x04, 0x00,       // no context engine id
...
int Snmp::broadcast_discovery(UdpAddressCollection& addresses,
    const int timeout_sec, const UdpAddress& addr, const snmp_version version,
    const OctetStr* community)
{
    ...
    do {
        ...
        if (something_to_receive)
        {
            ...
            OctetStr   engine_id;
            Pdu        dummy_pdu;
            if (receive_snmp_response(sock, *this, dummy_pdu, from, engine_id,
                    false /* process_msg */)
                == SNMP_CLASS_SUCCESS)
            {
                addresses += from;
...
#    elif defined(CPU) && CPU == PPC603
    m_hThread = taskSpawn("Snmp::process_thread", 0, 0, 10000,
        (int (*)(...))Snmp::process_thread, (int)this, 0, 0, 0, 0, 0, 0, 0, 0,
        0);
    if (m_hThread == ERROR)
    {
