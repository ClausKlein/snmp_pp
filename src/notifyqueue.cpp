...
#endif
//--------[ externs ]---------------------------------------------------
extern int receive_snmp_notification(
...
CNotifyEventQueue::CNotifyEventQueueElt::CNotifyEventQueueElt(
    CNotifyEvent* notifyevent, CNotifyEventQueueElt* next,
...
SnmpSocket CNotifyEventQueue::get_notify_fd() const { return m_notify_fd; }
    ...
    Snmp* snmp, const OidCollection& trapids, const TargetCollection& targets)
{
    ...
    {
        debugprintf(
            0, "WARNING: Adding notification event for other Snmp object");
    }
    ...
    if (!m_msgCount)
    {
        ...
        if (is_v4_address)
        {
            ...
            // bind the socket
            if (bind(
                    m_notify_fd, (struct sockaddr*)&mgr_addr, sizeof(mgr_addr))
                < 0)
            {
#ifdef WIN32
                int werr = WSAGetLastError();
...
#    ifdef NOTIFY_SET_IPV6_V6ONLY
            int on = 1;
            if (setsockopt(m_notify_fd, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&on,
                    sizeof(on))
                == -1)
            {
                LOG_BEGIN(loggerModuleName, WARNING_LOG | 1);
                LOG("Could not set option IPV6_V6ONLY on notify socket "
                    "(errno)");
                LOG(errno);
                LOG_END;
            }
...
#    endif
            ...
            OctetStr addrstr =
                ((IpAddress&)m_notify_addr).IpAddress::get_printable();
...
bool CNotifyEventQueue::GetFdArray(struct pollfd* readfds, int& remaining)
{
    SnmpSynchronize _synchronize(*this); // instead of REENTRANT()
...
int CNotifyEventQueue::HandleEvents(
    const struct pollfd* readfds, const int fds)
{
    SnmpSynchronize _synchronize(*this); // instead of REENTRANT()
    ...
    for (int i = 0; i < fds; i++)
    {
        ...
        if ((SNMP_CLASS_SUCCESS == status) || (SNMP_CLASS_TL_FAILED == status))
        {
