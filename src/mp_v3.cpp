...
#        define BEGIN_REENTRANT_CODE_BLOCK_CONST \
            SnmpSynchronize auto_lock(           \
                *(PP_CONST_CAST(SnmpSynchronized*, &lock)))
#    else
...
int v3MP::EngineIdTable::delete_entry(const OctetStr& host, int port)
{
    ...
    {
        LOG_BEGIN(loggerModuleName, WARNING_LOG | 4);
        LOG("v3MP::EngineIdTable: cannot remove nonexisting entry (host) "
            "(port)");
        LOG(host.get_printable());
        LOG(port);
        LOG_END;
    ...
    unsigned long req_id, int msg_id, bool local_request)
{
    ...
    for (int i = 0; i < entries; i++)
        ...
        {
            LOG_BEGIN(loggerModuleName, INFO_LOG | 8);
            LOG("v3MP::Cache: Delete unprocessed entry (n) (req id) (msg id) "
                "(type)");
            LOG(i);
            LOG(req_id);
            LOG(msg_id);
...
int v3MP::send_report(unsigned char* scopedPDU, int scopedPDULength,
    struct snmp_pdu* pdu, int errorCode, int sLevel, int sModel,
    OctetStr& sName, UdpAddress& destination, Snmp* snmp_session)
{
    ...
    if (scopedPDULength != MAX_SNMP_PACKET)
    {
        ...
        unsigned char* data =
            asn1_parse_scoped_pdu(scopedPDU, &scopedPDULength, cEngineID,
                &cEngineIDLength, cName, &cNameLength);
        if (!data)
        {
    ...
    case SNMPv3_USM_UNSUPPORTED_SECURITY_LEVEL: {
        ...
        counterVb.set_value(
            Counter32(usm->get_stats_unsupported_sec_levels()));
        sLevel = SNMP_SECURITY_LEVEL_NOAUTH_NOPRIV;
        break;
    }
    default: {
    ...
    status = snmp_build(pdu, sendbuffer.get_ptr(), &sendbufferlen,
        ...
        OctetStr(cEngineID, cEngineIDLength), OctetStr(cName, cNameLength));
    if (status != SNMPv3_MP_OK)
    {
        debugprintf(2,
            "v3MP::send_report: error serializing message (mpSnmpBuild "
            "returns: %i).",
            status);
        return SNMPv3_MP_ERROR;
    }
    ...
    {
        debugprintf(
            1, "v3MP::send_report: error sending message (%i)", status);
        return SNMPv3_MP_ERROR;
    }
...
int v3MP::snmp_parse(Snmp* snmp_session, struct snmp_pdu* pdu,
    unsigned char* inBuf, int inBufLength, OctetStr& securityEngineID,
    OctetStr& securityName, OctetStr& contextEngineID, OctetStr& contextName,
    SmiINT32& securityLevel, SmiINT32& msgSecurityModel,
    snmp_version& spp_version, UdpAddress from_address)
{
    ...
    securityParametersPosition =
        SAFE_INT_CAST(inBuf - inBufPtr) - msgSecurityParametersLength;
    ...
    debugprintf(3,
        "Parsed msgdata length(0x%x), "
        "msgSecurityParameters length(0x%x)",
        msgDataLength, msgSecurityParametersLength);
    ...
    case SNMP_SECURITY_MODEL_USM: {
        rc = usm->process_msg(msgMaxSize, msgSecurityParameters.get_ptr(),
            msgSecurityParametersLength, securityParametersPosition,
            securityLevel, inBufPtr, origLength, // wholeMsg
            msgData.get_ptr(), msgDataLength, securityEngineID, securityName,
            scopedPDU.get_ptr(), &scopedPDULength, &maxSizeResponseScopedPDU,
            securityStateReference, from_address);
    ...
    if (!errorCode)
    {
        unsigned char* data = asn1_parse_scoped_pdu(scopedPDUPtr,
            &scopedPDULength, tmp_contextEngineID, &tmp_contextEngineIDLength,
        ...
        int dataLength = scopedPDULength;
        contextEngineID.set_data(
            tmp_contextEngineID, tmp_contextEngineIDLength);
                ...
                || (pdu->command == TRP2_REQ_MSG)))
        {
            ...
            debugprintf(2,
                "mp: received request message with zero length"
    ...
    if (errorCode)
    {
        if ((reportableFlag) && (errorCode != SNMPv3_USM_PARSE_ERROR))
        {
            ...
            send_report(scopedPDUPtr, scopedPDULength, pdu, errorCode,
                securityLevel, msgSecurityModel, securityName, from_address,
                snmp_session);
...
#    ifdef BUGGY_REPORT_REQID
        ...
        {
            debugprintf(0,
                "WARNING: setting reqid of REPORT PDU (from) (to): (%ld) "
                "(%ld)",
                pdu->reqid, centry.req_id);
            pdu->reqid = centry.req_id;
        }
        ...
        || (pdu->command == INFORM_REQ_MSG))
    {
        ...
        else
        {
            if (!(unsignedCharCompare(securityEngineID.data(),
                    securityEngineID.len(), own_engine_id, own_engine_id_len)))
            {
                debugprintf(0,
                ...
                if (usm->is_discovery_enabled())
                {
                    ...
                    if (reportableFlag)
                    {
                        ...
                        send_report(0, MAX_SNMP_PACKET, pdu,
                            ...
                            snmp_session);
                        clear_pdu(pdu,
                            true); // Clear pdu and free all content AND IDs!
                    }
                    else
                    {
                        usm->delete_sec_state_reference(
                            securityStateReference);
                    }
...
int v3MP::snmp_build(struct snmp_pdu* pdu, unsigned char* packet,
    int*            out_length, // maximum Bytes in packet
    const OctetStr& securityEngineID, const OctetStr& securityName,
    int securityModel, int securityLevel, const OctetStr& contextEngineID,
...
#    endif
    ...
    LOG("v3MP: Building message with (SecurityEngineID) (securityName) "
    ...
    bufPtr =
        build_data_pdu(pdu, bufPtr, &maxLen, scopedPDUPtr, scopedPDULength);
    ...
    scopedPDUPtr = asn1_build_scoped_pdu(scopedPDUPtr, &maxLen,
        contextEngineID.data(), contextEngineID.len(), contextName.data(),
        contextName.len(), buf.get_ptr(), bufLength);
    ...
    globalDataLength =
        SAFE_INT_CAST(globalDataPtr - (unsigned char*)&globalData);
    ...
    case SNMP_SECURITY_MODEL_USM: {
        ...
        rc = usm->generate_msg(globalData, globalDataLength, *out_length,
            (use_own_engine_id ? own_engine_id_oct : securityEngineID),
            securityName, securityLevel, scopedPDU.get_ptr(), scopedPDULength,
