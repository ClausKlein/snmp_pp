...
#        define DES_CBC_ENCRYPT(pt, ct, s, iv, l) \
            DES_ncbc_encrypt(                     \
                pt, ct, l, &(s), (const_DES_cblock*)(iv), DES_ENCRYPT)
#        define DES_CBC_DECRYPT(ct, pt, s, iv, l) \
            DES_ncbc_encrypt(                     \
                ct, pt, l, &(s), (const_DES_cblock*)(iv), DES_DECRYPT)

#        define DES_EDE3_CBC_ENCRYPT(pt, ct, l, k1, k2, k3, iv)  \
            DES_ede3_cbc_encrypt(pt, ct, l, &(k1), &(k2), &(k3), \
                (const_DES_cblock*)(iv), DES_ENCRYPT)

#        define DES_EDE3_CBC_DECRYPT(ct, pt, l, k1, k2, k3, iv)  \
            DES_ede3_cbc_encrypt(ct, pt, l, &(k1), &(k2), &(k3), \
...
#            define DES_EDE3_CBC_ENCRYPT(pt, ct, l, k1, k2, k3, iv)         \
                des_ede3_cbc_encrypt((C_Block*)(pt), (C_Block*)(ct), l, k1, \
                    k2, k3, (C_Block*)(iv), DES_ENCRYPT)

#            define DES_EDE3_CBC_DECRYPT(ct, pt, l, k1, k2, k3, iv)         \
                des_ede3_cbc_encrypt((C_Block*)(ct), (C_Block*)(pt), l, k1, \
                    k2, k3, (C_Block*)(iv), DES_DECRYPT)

#            ifdef RSAEURO

#                undef MD5_PROCESS
#                define MD5_PROCESS(s, p, l) \
                    MD5Update(s, (unsigned char*)(p), l)
...
#                define DES_CBC_ENCRYPT(pt, ct, s, iv, l) \
                    DES_CBCUpdate(&(s), pt, ct, l)
#                define DES_CBC_DECRYPT(ct, pt, s, iv, l) \
                    DES_CBCUpdate(&(s), (unsigned char*)(ct), pt, l)
...
#                define DES_CBC_ENCRYPT(pt, ct, s, iv, l)                  \
                    des_ncbc_encrypt((C_Block*)(pt), (C_Block*)(ct), l, s, \
                        (C_Block*)(iv), DES_ENCRYPT)
#                define DES_CBC_DECRYPT(ct, pt, s, iv, l)                  \
                    des_ncbc_encrypt((C_Block*)(ct), (C_Block*)(pt), l, s, \
...
AuthPriv::AuthPriv(int& construct_state)
{
    ...
    {
        LOG_BEGIN(loggerModuleName, ERROR_LOG | 1);
        LOG("AuthPriv: *BUG* sizeof(pp_uint64) is not 8 bytes. snmp++ has to "
            "be patched for this system.");
        LOG_END;
...
#    ifdef __DEBUG
    debughexcprintf(
        21, "keychange_value", keychange_value.data(), keychange_value.len());
#    endif
...
int AuthPriv::password_to_key_auth(const int auth_prot,
    const unsigned char* password, const unsigned int password_len,
    const unsigned char* engine_id, const unsigned int engine_id_len,
...
int AuthPriv::password_to_key_priv(const int auth_prot, const int priv_prot,
    const unsigned char* password, const unsigned int password_len,
    const unsigned char* engine_id, const unsigned int engine_id_len,
    unsigned char* key, unsigned int* key_len)
{
...
int AuthPriv::encrypt_msg(const int priv_prot, const unsigned char* key,
...
int AuthPriv::decrypt_msg(const int priv_prot, const unsigned char* key,
    const unsigned int key_len, const unsigned char* buffer,
    const unsigned int buffer_len, unsigned char* out_buffer,
...
int AuthPriv::auth_out_msg(const int auth_prot, const unsigned char* key,
...
int AuthPriv::auth_inc_msg(const int auth_prot, const unsigned char* key,
    unsigned char* msg, const int msg_len, unsigned char* auth_par_ptr,
...
#    ifdef __DEBUG
    debugprintf(
        5, "password: %s.", OctetStr(password, password_len).get_printable());
...
int AuthMD5::hash(const unsigned char* data, const unsigned int data_len,
...
int AuthMD5::auth_inc_msg(const unsigned char* key, unsigned char* msg,
    const int msg_len, unsigned char* auth_par_ptr, const int auth_par_len)
{
    ...
    {
        debugprintf(4,
            "MD5 illegal digest length (%d), authentication FAILED.",
            auth_par_len);
...
#    endif
    ...
    {
        debugprintf(
            4, "Buffer too small: should be 8, is (%i).", *privacy_params_len);
    ...
    *privacy_params_len = 8;
    ...
    {
        privacy_params[3 - j] =
            (unsigned char)(0xFF & (engine_boots >> (8 * j)));
...
#    ifdef __DEBUG
    debughexcprintf(
        21, "apDESEncryptData: Data to encrypt", buffer, buffer_len);
    debughexcprintf(
        21, "apDESEncryptData: used key (only 8 bytes used)", key, 16);
    debughexcprintf(21, "apDESEncryptData: used iv", initVect, 8);
#    endif
...
#    ifdef __DEBUG
    debughexcprintf(
        21, "apDESEncryptData: created privacy_params", privacy_params, 8);
    debughexcprintf(
        21, "apDESEncryptData: encrypted Data", out_buffer, *out_buffer_len);
#    endif

    return SNMPv3_USM_OK;
}

int PrivDES::decrypt(const unsigned char* key, const unsigned int /*key_len*/,
    const unsigned char* buffer, const unsigned int buffer_len,
...
#    ifdef __DEBUG
    debughexcprintf(
        21, "apDESDecryptData: Data to decrypt", buffer, buffer_len);
    debughexcprintf(
        21, "apDESDecryptData: used key (only 8 bytes used)", key, 16);
    debughexcprintf(
        21, "apDESDecryptData: used privacy_params", privacy_params, 8);
    debughexcprintf(21, "apDESDecryptData: used iv", initVect, 8);
#    endif
...
#    ifdef __DEBUG
    debughexcprintf(
        21, "apDESDecryptData: decrypted Data", outBuffer, *outBuffer_len);
#    endif
...
int PrivIDEA::encrypt(const unsigned char* key, const unsigned int /*key_len*/,
    const unsigned char* buffer, const unsigned int buffer_len,
    unsigned char* out_buffer, unsigned int* out_buffer_len,
    unsigned char* privacy_params, unsigned int* privacy_params_len,
    const uint32_t engine_boots, const uint32_t /*engine_time*/)
...
#        endif
    ...
    *privacy_params_len = 8;
    ...
    idea_cfb_encrypt(&CFB_Context, iv, out_buffer, buffer, buffer_len);
...
#        ifdef __DEBUG
    debughexcprintf(
        21, "apIDEAEncryptData: Data to encrypt", buffer, buffer_len);
    debughexcprintf(21, "apIDEAEncryptData: key", key, 16);
    debughexcprintf(
        21, "apIDEAEncryptData: privacy_params", privacy_params, 8);
    debughexcprintf(
        21, "apIDEAEncryptData: encrypted Data", out_buffer, *out_buffer_len);
#        endif

    return SNMPv3_USM_OK;
}

int PrivIDEA::decrypt(const unsigned char* key, const unsigned int /*key_len*/,
    const unsigned char* buffer, const unsigned int buffer_len,
    unsigned char* out_buffer, unsigned int* out_buffer_len,
    const unsigned char* privacy_params, const unsigned int privacy_params_len,
    const uint32_t /*engine_boots*/, const uint32_t /*engine_time*/)
...
#        ifdef __DEBUG
    debughexcprintf(
        21, "apIDEADecryptData: Data to decrypt", buffer, buffer_len);
    debughexcprintf(21, "apIDEADecryptData: key", key, 16);
    debughexcprintf(
        21, "apIDEAEncryptData: privacy_params", privacy_params, 8);
    debughexcprintf(
        21, "apIDEADecryptData: decrypted Data", out_buffer, *out_buffer_len);
#        endif
...
PrivAES::PrivAES(const int aes_type_) : aes_type(aes_type_)
{
#        if defined(_USE_LIBTOMCRYPT) && !defined(_USE_OPENSSL)
    ...
    const uint32_t engine_time)
{
    ...
    {
        debugprintf(0, "Output buffer too small (%d < %d)", *out_buffer_len,
            buffer_len);
        return SNMPv3_USM_ENCRYPTION_ERROR;
    }
...
#        ifdef _USE_OPENSSL
    ...
    {
        debugprintf(1, "Encryption wrote (%d + %d) bytes instead of (%d)",
            len1, len2, buffer_len);
        return SNMPv3_USM_ENCRYPTION_ERROR;
    }
#        else
...
#        ifdef __DEBUG
    debughexcprintf(
        21, "aes EncryptData: Data to encrypt", buffer, buffer_len);
    debughexcprintf(21, "aes EncryptData: used key", key, key_len);
    debughexcprintf(
        21, "aes EncryptData: created privacy_params", privacy_params, 8);
    debughexcprintf(
        21, "aes EncryptData: encrypted Data", out_buffer, *out_buffer_len);
#        endif
...
#        ifdef __DEBUG
    debughexcprintf(
        21, "aes DecryptData: Data to decrypt", buffer, buffer_len);
    debughexcprintf(21, "aes DecryptData: used key", key, key_len);
    debughexcprintf(
        21, "aes DecryptData: used privacy_params", privacy_params, 8);
    debughexcprintf(
        21, "aes DecryptData: decrypted Data", out_buffer, *out_buffer_len);
#        endif
    ...
    unsigned int* key_len, const unsigned int max_key_len, Auth* auth)
{
    ...
    {
        debugprintf(
            0, "Out of mem. Did not get %i bytes.", auth->get_hash_len());
        return SNMPv3_USM_ERROR;
    }
...
int PrivAESW3DESKeyExt::extend_short_key(const unsigned char* password,
    ...
    unsigned int* key_len, const unsigned int max_key_len, Auth* auth)
{
    return des3_extend_short_key(password, password_len, engine_id,
        engine_id_len, key, key_len, max_key_len, auth,
...
int Priv3DES_EDE::decrypt(const unsigned char* key, const unsigned int key_len,
    const unsigned char* buffer, const unsigned int buffer_len,
    unsigned char* out_buffer, unsigned int* out_buffer_len,
    const unsigned char* privacy_params, const unsigned int privacy_params_len,
    const uint32_t /* engine_boots */, const uint32_t /* engine_time */)
...
int Priv3DES_EDE::extend_short_key(const unsigned char* password,
    const unsigned int password_len, const unsigned char* engine_id,
    const unsigned int engine_id_len, unsigned char* key,
    unsigned int* key_len, const unsigned int max_key_len, Auth* auth)
{
...
bool Priv3DES_EDE::test()
{
    ...
    debughexcprintf(1, "result key 3DES SHA", key, key_len);
    ...
    status = ap.encrypt_msg(SNMP_PRIVPROTOCOL_3DESEDE, key, key_len, msg, 53,
        enc_buffer, &enc_buffer_len, priv_params, &priv_params_len, 0x5abc,
        0x6def);

    debughexcprintf(1, "encrypted text", enc_buffer, enc_buffer_len);

    status = ap.decrypt_msg(SNMP_PRIVPROTOCOL_3DESEDE, key, key_len,
        enc_buffer, enc_buffer_len, dec_buffer, &dec_buffer_len, priv_params,
        priv_params_len, 0x5abc, 0x6def);

    dec_buffer[dec_buffer_len] = 0;
    debugprintf(1, "decrypted text: %s", dec_buffer);
...
int AuthSHABase::hash(const unsigned char* data, const unsigned int data_len,
    unsigned char* digest) const
{
    std::unique_ptr<Hasher> h(get_hasher());
...
int AuthSHABase::auth_out_msg(const unsigned char* key, unsigned char* msg,
    const int msg_len, unsigned char* auth_par_ptr)
{
    std::unique_ptr<Hasher> h(get_hasher());
    ...
    unsigned char*          k_ipad =
        ipad.get_ptr(); /* inner padding - key XORd with ipad */
    unsigned char* k_opad =
        opad.get_ptr(); /* outer padding - key XORd with opad */
...
int AuthSHABase::auth_inc_msg(const unsigned char* key, unsigned char* msg,
    const int msg_len, unsigned char* auth_par_ptr, const int auth_par_len)
{
    ...
    {
        debugprintf(4,
            "SHA illegal digest length (%d), expected (%d), authentication "
            "FAILED.",
...
public:
    ...
    int update(const unsigned char* data, const unsigned int data_len) override
    {
