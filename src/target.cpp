...
CTarget::CTarget() : read_community(PUBLIC), write_community(PUBLIC)
{
...
CTarget::CTarget(const Address& address, const char* read_community_name,
    const char* write_community_name)
    : SnmpTarget(address), read_community(read_community_name),
...
//-----------[ CTarget:: CTarget ]-----------------------------------
// CTarget constructor with args
CTarget::CTarget(const Address& address, const OctetStr& read_community_name,
...
// CTarget constructor with args
CTarget::CTarget(const CTarget& target)
    : SnmpTarget(), read_community(target.read_community),
...
int CTarget::operator==(const CTarget& rhs) const
{
    ...
    if (read_community != rhs.read_community)
    {
        return 0;
    }
    if (write_community != rhs.write_community)
    {
        return 0;
    }
    return 1; // equal
...
UTarget::UTarget(
    const Address& address, const char* sec_name, const int sec_model)
...
// UTarget constructor with args
UTarget::UTarget(
    const Address& address, const OctetStr& sec_name, const int sec_model)
    : SnmpTarget(address), security_name(sec_name), security_model(sec_model)
#ifdef _SNMPv3
      ,
      engine_id("")
...
// UTarget constructor with args
UTarget::UTarget(const UTarget& target)
    : SnmpTarget(), security_name(target.security_name),
...
int UTarget::operator==(const UTarget& rhs) const
{
    ...
    if (security_name != rhs.security_name)
    {
        return 0;
    }
    if (security_model != rhs.security_model)
    {
        return 0;
    }
    return 1; // they are equal
