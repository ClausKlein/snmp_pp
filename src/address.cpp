...
#include "snmp_pp/v3.h" // for debugprintf()
...
#    define ADDRESS_TRACE \
        debugprintf(0, "ADDRESS %p Enter %s", this, __PRETTY_FUNCTION__)
#    define ADDRESS_TRACE2 \
        debugprintf(0, "ADDRESS op Enter %s", __PRETTY_FUNCTION__)
#else
#    define ADDRESS_TRACE
#    define ADDRESS_TRACE2
#endif
...
#    if !(defined(HAVE_GETHOSTBYADDR_R)          \
        || defined(HAVE_REENTRANT_GETHOSTBYADDR) \
...
#    if !(defined(HAVE_GETHOSTBYNAME_R)          \
        || defined(HAVE_REENTRANT_GETHOSTBYNAME) \
        || defined(HAVE_GETADDRINFO))
// If you see this warning, and your system has a reentrant localtime
// or localtime_r function report your compiler, OS,... to the authors
// of this library, so that these settings can be changed
...
Address::Address() : addr_changed(true), valid_flag(false)
{
    ADDRESS_TRACE;
...
int operator>=(const Address& lhs, const char* rhs)
{
    ...
    if (!rhs) return true; // always >= NULL
    if (strcmp((const char*)lhs, rhs) >= 0) return true;
    return false;
}
...
int operator<(const Address& lhs, const char* rhs)
{
    ...
    if (!rhs) return false; // always >= NULL
    if (strcmp((const char*)lhs, rhs) < 0) return true;
    return false;
}
...
int operator<=(const Address& lhs, const char* rhs)
{
    ...
    if (!rhs) return !lhs.valid(); // invalid == NULL, else valid > NULL
    if (strcmp((const char*)lhs, rhs) <= 0) return true;
    return false;
}
...
IpAddress::IpAddress(const GenAddress& genaddr) : iv_friendly_name_status(0)
{
    ADDRESS_TRACE;
...
SnmpSyntax& IpAddress::operator=(const SnmpSyntax& val)
{
    ...
    if (val.valid())
    {
        ...
        case sNMP_SYNTAX_OCTETS:
            ...
            else if ((((IpAddress&)val).smival.value.string.len
                         == IP6LEN_NO_SCOPE)
...
IpAddress& IpAddress::operator=(const IpAddress& ipaddr)
{
    ...
    if (valid_flag)
    {
        ...
        else
        {
            ...
            {
                memcpy(
                    address_buffer, ipaddr.address_buffer, IP6LEN_WITH_SCOPE);
                ip_version              = version_ipv6;
                smival.value.string.len = IP6LEN_WITH_SCOPE;
                have_ipv6_scope         = true;
            }
...
int IpAddress::parse_coloned_ipstring(const char* inaddr)
{
    ...
    if (second_used)
    {
        ...
        for (i = 0; i < IP6LEN_NO_SCOPE - (len_first + len_second); i++)
            *end_first_part++ = 0;
        for (i = 0; i < len_second; i++) *end_first_part++ = second[i];
    }
...
#    if SNMP_PP_IPv6
        if (res->ai_family == AF_INET6)
        {
            if (!inet_ntop(AF_INET6,
                    &((struct sockaddr_in6*)(res->ai_addr))->sin6_addr, ds,
                    sizeof(ds) - 1))
            {
                freeaddrinfo(res);
                return false;
...
#    endif
            if (res->ai_family == AF_INET)
        {
            ...
            if (!inet_ntop(AF_INET,
                    &((struct sockaddr_in*)(res->ai_addr))->sin_addr, ds,
                    sizeof(ds) - 1))
            {
                freeaddrinfo(res);
                return false;
...
#            else
    int tmp_ret = gethostbyname_r(
        inaddr, &lookup_buf, buf, 2048, &lookupResult, &herrno);
    if (tmp_ret)
    {
        debugprintf(
            1, "Error (%d, errno %d) from gethostbyname_r", tmp_ret, herrno);
        lookupResult = 0;
    }
#            endif
#            ifdef SNMP_PP_IPv6
    ...
    {
#                ifdef __sun
        lookupResult = gethostbyname_r(
            inaddr, AF_INET6, &lookup_buf, buf, 2048, &lookupResult, &herrno);
#                else
        int tmp_ret2 = gethostbyname2_r(
            inaddr, AF_INET6, &lookup_buf, buf, 2048, &lookupResult, &herrno);
        if (tmp_ret2)
        {
...
#        ifdef SNMP_PP_IPv6
        if (lookupResult->h_length == sizeof(in6_addr))
        {
            ...
            in6_addr ipAddr;
            memcpy(
                (void*)&ipAddr, (void*)lookupResult->h_addr, sizeof(in6_addr));

            // now lets check out the coloned string
            if (!inet_ntop(AF_INET6, &ipAddr, ds, 60)) return false;
            debugprintf(4, "from inet_ntop: %s", ds);
            if (!parse_coloned_ipstring(ds)) return false;
...
#        else
        gethostbyaddr_r((char*)&ipAddr, sizeof(in_addr), AF_INET, &lookup, buf,
...
#            else
        gethostbyaddr_r((char*)&ipAddr, sizeof(in_addr), AF_INET6, &lookup,
...
bool IpAddress::map_to_ipv6()
{
    ...
    if (ip_version != version_ipv4) return false;
...
UdpAddress& UdpAddress::operator=(const UdpAddress& udpaddr)
{
    ...
    if (udpaddr.addr_changed) { addr_changed = true; }
    else
    {
        memcpy(output_buffer, udpaddr.output_buffer,
            sizeof(unsigned char) * OUTBUFF_UDP);
...
bool UdpAddress::parse_address(const char* inaddr)
{
    ...
    bool found           = false;
    int  pos =
        (int)strlen(buffer) - 1; // safe to cast as max is MAX_FRIENDLY_NAME
    bool do_loop             = true;
    ...
    while (do_loop)
    {
        if (buffer[pos] == '/')
        {
            found = true;
            sep   = '/';
            if (buffer[pos - 1] == ']') remove_brackets = true;
            break;
        }
        if (buffer[pos] == ':')
        {
            ...
            {
                if (buffer[i] == ':') another_colon_found = true;
                if (buffer[i] == '%') scope_found = true;
            }
        ...
        pos--;
        do_loop =
            ((found == false) && (pos >= 0) && (another_colon_found == false));
    }
...
unsigned short UdpAddress::get_port() const
{
    ...
    if (valid_flag)
    {
        ...
        if (ip_version == version_ipv4)
            ...
            port_nbo =
                (const unsigned short*)(address_buffer + IP6LEN_WITH_SCOPE);
        else
            port_nbo =
                (const unsigned short*)(address_buffer + IP6LEN_NO_SCOPE);
...
IpxAddress::IpxAddress(const IpxAddress& ipxaddr)
{
    ...
    if (valid_flag) memcpy(address_buffer, ipxaddr.address_buffer, IPXLEN);
    addr_changed = true;
}
...
IpxAddress::IpxAddress(const GenAddress& genaddr)
{
    ...
    if (valid_flag)
    {
        ...
        {
            *this =
                genaddr.cast_ipxsockaddress(); // copy in the Ipx address data
        }
        else
            valid_flag = false;
    }
...
SnmpSyntax& IpxAddress::operator=(const SnmpSyntax& val)
{
    ...
    if (val.valid())
    {
        ...
        case sNMP_SYNTAX_OCTETS:
            ...
            {
                memcpy(address_buffer,
                    ((IpxAddress&)val).smival.value.string.ptr, IPXLEN);
                valid_flag = true;
            }
            break;
        }
...
IpxAddress& IpxAddress::operator=(const IpxAddress& ipxaddress)
{
    ...
    if (valid_flag) memcpy(address_buffer, ipxaddress.address_buffer, IPXLEN);
    addr_changed = true;
    return *this;
}
...
bool IpxAddress::parse_address(const char* inaddr)
{
    ...
    if ((tmplen < 21) || (tmplen > 22)) return false;

    // convert the string to all lower case
    // this allows hex values to be in upper or lower
    for (z = 0; z < tmplen; z++) temp[z] = tolower(temp[z]);

    // check for separated nodeid
    // if found remove it
    if (temp[15] == '-')
    {
        for (z = 16; z < tmplen; z++) temp[z - 1] = temp[z];
        temp[tmplen - 1] = 0;
    }
...
SnmpSyntax& IpxSockAddress::operator=(const SnmpSyntax& val)
{
    ...
    if (val.valid())
    {
        ...
        case sNMP_SYNTAX_OCTETS: {
            ...
            else if (((IpxSockAddress&)val).smival.value.string.len
                == IPXSOCKLEN)
            {
                memcpy(address_buffer,
...
MacAddress::MacAddress(const MacAddress& macaddr)
{
    ...
    if (valid_flag) memcpy(address_buffer, macaddr.address_buffer, MACLEN);
    addr_changed = true;
}
...
MacAddress& MacAddress::operator=(const MacAddress& macaddress)
{
    ...
    if (valid_flag) memcpy(address_buffer, macaddress.address_buffer, MACLEN);
    addr_changed = true;
    return *this;
}
...
bool MacAddress::parse_address(const char* inaddr)
{
    ...
    // bad total length check
    if (strlen(temp) != 17) return false;
    ...
    // convert to lower
    for (z = 0; z < strlen(temp); z++) temp[z] = tolower(temp[z]);
...
//-----------[ constructor with a string argument ]----------------------
GenAddress::GenAddress(const char* addr, const Address::addr_type use_type)
{
    ADDRESS_TRACE;
    ...
    const char* addr, const Address::addr_type use_type)
{
    ...
    if ((use_type == Address::type_invalid) || (use_type == Address::type_udp))
    {
        // udp address
...
#ifdef _MAC_ADDRESS
    if ((use_type == Address::type_invalid) || (use_type == Address::type_mac))
    {
        // mac address
