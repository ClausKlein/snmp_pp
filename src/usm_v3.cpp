...
public:
     ...
     */
    int add_entry(const OctetStr& engine_id, const SmiINT32 engine_boots,
     ...
     */
    int get_time(const OctetStr& engine_id, SmiINT32& engine_boots,
     ...
     */
    int check_time(const OctetStr& engine_id, const SmiINT32 engine_boots,
        const SmiINT32 engine_time);
...
public:
     ...
     */
    int add_entry(const OctetStr& user_name, const OctetStr& security_name,
        const SmiINT32 auth_proto, const SmiINT32 priv_proto,
        const OctetStr& auth_pass, const OctetStr& priv_pass);
     ...
     */
    const struct UsmUserNameTableEntry* get_entry(
        const OctetStr& security_name);

    /**
     * Get a clone of the entry with the given securityName from the
     * usmUserNameTable
     *
     ...
     */
    struct UsmUserNameTableEntry* get_cloned_entry(
        const OctetStr& security_name);
...
public:
     ...
     */
    const struct UsmUserTableEntry* get_entry(
     ...
     *           found
     */
    struct UsmUserTableEntry* get_cloned_entry(
        const OctetStr& engine_id, const OctetStr& sec_name);
     ...
     */
    int add_entry(const OctetStr& engine_id, const OctetStr& user_name,
        const OctetStr& sec_name, const SmiINT32 auth_proto,
        const OctetStr& auth_key, const SmiINT32 priv_proto,
     ...
     *           SNMPv3_USM_OK
     */
    int update_key(const OctetStr& user_name, const OctetStr& engine_id,
        const OctetStr& new_key, const int key_type);
...
struct SecurityStateReference {
    unsigned char  msgUserName[MAXLEN_USMUSERNAME];
    int            msgUserNameLength;
    unsigned char* securityName;
    int            securityNameLength;
    unsigned char* securityEngineID;
    int            securityEngineIDLength;
    int            authProtocol;
...
USM::USM(unsigned int engine_boots, const OctetStr& engine_id,
    ...
    : local_snmp_engine_id(engine_id), v3mp(v3_mp),
      ...
      usmStatsUnsupportedSecLevels(0), usmStatsNotInTimeWindows(0),
...
int USM::add_usm_user(const OctetStr& user_name, const OctetStr& security_name,
    const SmiINT32 auth_protocol, const SmiINT32 priv_protocol,
    const OctetStr& auth_password, const OctetStr& priv_password)
{
    ...
    unsigned int* priv_key_len)
{
    ...
    res = auth_priv->password_to_key_priv(auth_prot, priv_prot, priv_password,
        priv_password_len, engine_id.data(), engine_id.len(), priv_key,
        priv_key_len);
...
struct UsmUser* USM::get_user(
    const OctetStr& engine_id, const OctetStr& security_name)
{
    ...
    {
        name_table_entry =
            usm_user_name_table->get_cloned_entry(security_name);
        if (!name_table_entry)
        {
            const struct UsmUserTableEntry* entry = nullptr;
        ...
        if (engine_id.len() == 0)
        {
            ...
            res->securityName =
                v3strcpy(name_table_entry->usmUserSecurityName.data(),
        ...
        else
        {
            ...
            int res = build_localized_keys(engine_id,
                ...
                name_table_entry->privPasswordLength, authKey, &authKeyLength,
            ...
            add_localized_user(engine_id, name_table_entry->usmUserName,
            ...
            if (usm_add_user_cb)
            {
                ...
                usm_add_user_cb(engine_id, name_table_entry->usmUserName,
                    name_table_entry->usmUserSecurityName,
                    name_table_entry->usmUserAuthProtocol, akey,
                    name_table_entry->usmUserPrivProtocol, pkey);
            }
            ...
            if (!user_table_entry)
            {
                ...
                LOG("Get of just added localized entry failed (sec name) "
                    "(engine id)");
...
const struct UsmUserNameTableEntry* USM::get_user(
    const OctetStr& security_name)
{
    return usm_user_name_table->get_entry(security_name);
}

int USM::get_user_count() const { return usm_user_table->size(); }
...
AuthPriv* USM::get_auth_priv() { return auth_priv; }

struct UsmKeyUpdate* USM::key_update_prepare(const OctetStr& securityName,
    SnmpTarget& target, const OctetStr& newPassword, Pdu& pdu, int type,
    int& status, const OctetStr& oldpass, const OctetStr& oldengid,
    const OctetStr& newengid)
{
    ...
    OctetStr engineID = "";
    // get engineID
    if (v3mp->get_from_engine_id_table(
            engineID, (char*)udp_address.get_printable())
        != SNMPv3_MP_OK)
    {
        debugprintf(0,
    ...
    case AUTHKEY:
    case OWNAUTHKEY: {
        status = auth_priv->password_to_key_auth(user->authProtocol,
            newPassword.data(), newPassword.len(), engineID.data(),
            engineID.len(), key, &key_len);
    ...
    case OWNPRIVKEY: {
        status = auth_priv->password_to_key_priv(user->authProtocol,
            user->privProtocol, newPassword.data(), newPassword.len(),
            engineID.data(), engineID.len(), key, &key_len);
    ...
    auth_priv->get_keychange_value(
        user->authProtocol, oldKey, newKey, newValue);
...
void USM::key_update_abort(struct UsmKeyUpdate* uku) { delete uku; }
    ...
    int*           wholeMsgLength)     // OUT length of generated message
{
    ...
    if (securityStateReference)
    {
        ...
        else
        {
            ...
            user->engineID =
                v3strcpy(securityEngineID.data(), securityEngineID.len());
        }
        ...
        else
        {
            ...
            user->securityName =
                v3strcpy(securityName.data(), securityName.len());
            ...
            {
                memcpy(user->usmUserName, securityName.data(),
                    securityName.len());
        ...
        debugprintf(20,
            "securityStateReference: secName %d, authProt %d, akey %d",
            securityStateReference->securityNameLength,
    ...
    {
        debugprintf(0, "engine_id too long %i > %i", securityEngineID.len(),
            MAXLENGTH_ENGINEID);
        free_user(user);
        return SNMPv3_USM_ERROR;
    }
    ...
    {
        debugprintf(0, "user name too long %i > %i", user->usmUserNameLength,
            MAXLEN_USMUSERNAME);
        free_user(user);
        return SNMPv3_USM_ERROR;
    }
    ...
    memcpy(usmSecurityParams.msgUserName, user->usmUserName,
        user->usmUserNameLength);
    memcpy(usmSecurityParams.msgAuthoritativeEngineID, securityEngineID.data(),
    ...
    if (securityLevel >= SNMP_SECURITY_LEVEL_AUTH_NOPRIV)
    {
        ...
        rc = usm_time_table->get_time(securityEngineID,
    ...
    if (securityLevel == SNMP_SECURITY_LEVEL_AUTH_PRIV)
    {
        ...
        bufPtr = asn_build_string(bufPtr, &restLength,
            ASN_UNI_PRIM | ASN_OCTET_STR, buf2Ptr, buf2Length);
        if (!bufPtr)
        {
    ...
    if (securityLevel >= SNMP_SECURITY_LEVEL_AUTH_NOPRIV)
    {
        ...
        *wholeMsgLength = SAFE_INT_CAST(wholeMsgPtr - wholeMsg);
        ...
        rc = auth_priv->auth_out_msg(user->authProtocol, user->authKey,
            wholeMsg, *wholeMsgLength, wholeMsg + startAuthPar);

        if (rc != SNMPv3_USM_OK)
        {
            debugprintf(0,
                "usm: Authentication error for outgoing message."
                " error code (%i).",
                rc);
            delete_sec_parameters(&usmSecurityParams);
            free_user(user);
    ...
    const UdpAddress& fromAddress)
{
    ...
    debugprintf(3, "Parsed engineBoots(0x%lx), engineTime(0x%lx)", engineTime,
        engineBoots);
    ...
    debugprintf(5,
        "Parsed usmUserName length(0x%x)"
    ...
    {
        debugprintf(0, "user name too long: %i > %i.", usmUserNameLength,
            MAXLEN_USMUSERNAME);
        return SNMPv3_USM_PARSE_ERROR;
    }
    ...
    *scopedPDULength = msgDataLength;
    ...
    if ((security_engine_id.len() == 0)
        || (usm_time_table->check_engine_id(security_engine_id)
            != SNMPv3_USM_OK))
    {
    ...
    if (securityLevel > SNMP_SECURITY_LEVEL_NOAUTH_NOPRIV)
    {
        rc = auth_priv->auth_inc_msg(user->authProtocol, user->authKey,
            wholeMsg, wholeMsgLength, wholeMsg + authParametersPosition,
            authParamLength);
        if (rc != SNMPv3_USM_OK)
        {
            switch (rc)
            ...
            security_name.set_data(
                user->securityName, user->securityNameLength);
        ...
        rc = usm_time_table->check_time(
            security_engine_id, engineBoots, engineTime);
    ...
    *scopedPDULength = MAX_SNMP_PACKET;
    ...
    if ((securityLevel == SNMP_SECURITY_LEVEL_AUTH_PRIV) && (!notInTime))
    {
    ...
    *maxSizeResponseScopedPDU =
        maxMessageSize - (wholeMsgLength - *scopedPDULength);
...
unsigned char* USM::build_sec_params(unsigned char* outBuf, int* maxLength,
    struct UsmSecurityParameters sp, int* position)
{
    ...
    *position = SAFE_INT_CAST(bufPtr - buf.get_ptr()) + 2;
    ...
    debugprintf(
        5, "Coding sequence (securityPar), length = 0x%x", totalLength);
    outBufPtr =
...
void USM::lock_user_name_table() { usm_user_name_table->lock(); }
...
const UsmUserNameTableEntry* USM::peek_next_user(
    const UsmUserNameTableEntry* e)
{
    return usm_user_name_table->peek_next(e);
}

// Unlock the UsmUserNameTable after access through peek_first/next_user()
void USM::unlock_user_name_table() { usm_user_name_table->unlock(); }

// Lock the UsmUserTable for access through peek_first/next_luser()
void USM::lock_user_table() { usm_user_table->lock(); }
...
void USM::unlock_user_table() { usm_user_table->unlock(); }
...

{
            ...
            engine_id.data(), engine_id.len()))
    {
        ...
        if ((table[0].engine_boots == 2147483647)
            || (table[0].engine_boots != engine_boots)
            || (std::abs(static_cast<pp_int64>(now) + table[0].time_diff
                    - engine_time)
                > 150))
        {
            LOG_BEGIN(loggerModuleName, DEBUG_LOG | 9);
            LOG("USMTimeTable: Check time failed, authoritative (id) (boot) "
                "(time)");
            LOG(engine_id.get_printable());
            LOG(engine_boots);
            LOG(engine_time);
    ...
    for (int i = 1; i < entries; i++)
                ...
                engine_id.data(), engine_id.len()))
        {
            ...
            else
            {
                ...
                LOG_BEGIN(loggerModuleName, DEBUG_LOG | 9);
                LOG("USMTimeTable: Check time ok, not authoritative, updated "
                    "(id)");
                LOG(engine_id.get_printable());
                LOG_END;
...
int USMUserNameTable::add_entry(const OctetStr& user_name,
    const OctetStr& security_name, const SmiINT32 auth_proto,
    ...
    *user_name_len        = 0;
    ...
    for (int i = 0; i < entries; i++)
    {
        if (unsignedCharCompare(table[i].usmUserSecurityName.data(),
                table[i].usmUserSecurityName.len(), security_name,
                security_name_len))
        {
            if (buf_len < table[i].usmUserName.len())
            {
                LOG_BEGIN(loggerModuleName, ERROR_LOG | 1);
                LOG("USMUserNameTable: Buffer for user name too small (is) "
                    "(should)");
                LOG(buf_len);
...
int USMUserNameTable::save_to_file(const char* name, AuthPriv* ap)
{
...

    {
        ...
        for (int i = 0; i < entries; ++i)
        {
            ...
            encodeString(table[i].usmUserName.data(),
                table[i].usmUserName.len(), encoded);
            encoded[2 * table[i].usmUserName.len()] = '\n';
            if (fwrite(
                    encoded, 2 * table[i].usmUserName.len() + 1, 1, file_out)
                != 1)
            {
                failed = true;
                break;
            ...
            encodeString(
                table[i].authPassword, table[i].authPasswordLength, encoded);
            encoded[2 * table[i].authPasswordLength] = '\n';
            if (fwrite(
                    encoded, 2 * table[i].authPasswordLength + 1, 1, file_out)
                != 1)
            {
                failed = true;
                break;
            ...
            encodeString(
                table[i].privPassword, table[i].privPasswordLength, encoded);
            encoded[2 * table[i].privPasswordLength] = '\n';
            if (fwrite(
                    encoded, 2 * table[i].privPasswordLength + 1, 1, file_out)
                != 1)
            {
                failed = true;
                break;
...
int USMUserNameTable::load_from_file(const char* name, AuthPriv* ap)
{
    ...
    while (fgets((char*)line, MAX_LINE_LEN * 2, file_in))
    {
        ...
        LOG("USMUserNameTable: Adding user (user name) (sec name) (auth) "
            "(priv)");
...
USMUserTable::~USMUserTable()
{
    if (table)
    {
        for (int i = 0; i < entries; i++)
        {
            ...
            {
                memset(
                    table[i].usmUserAuthKey, 0, table[i].usmUserAuthKeyLength);
                delete[] table[i].usmUserAuthKey;
            }
            if (table[i].usmUserPrivKey)
            {
                memset(
                    table[i].usmUserPrivKey, 0, table[i].usmUserPrivKeyLength);
    ...
    *user_name_len = 0;
    ...
    for (int i = 0; i < entries; i++)
    {
        if (unsignedCharCompare(table[i].usmUserSecurityName,
                table[i].usmUserSecurityNameLength, sec_name, sec_name_len))
        {
            if (buf_len < table[i].usmUserNameLength)
            {
                LOG_BEGIN(loggerModuleName, ERROR_LOG | 1);
                LOG("USMUserTable: Buffer for user name too small (is) "
                    "(should)");
                LOG(buf_len);
            ...
            *user_name_len = table[i].usmUserNameLength;
            memcpy(
                user_name, table[i].usmUserName, table[i].usmUserNameLength);
    ...
    const long user_name_len, OctetStr& sec_name)
{
    ...
    for (int i = 0; i < entries; i++)
    {
        if (unsignedCharCompare(table[i].usmUserName,
                table[i].usmUserNameLength, user_name, user_name_len))
        {
    ...
    const OctetStr& engine_id, const OctetStr& sec_name)
{
    ...
    {
        res->usmUserEngineID =
            v3strcpy(e->usmUserEngineID, e->usmUserEngineIDLength);
        res->usmUserEngineIDLength = e->usmUserEngineIDLength;
        res->usmUserName = v3strcpy(e->usmUserName, e->usmUserNameLength);
        res->usmUserNameLength = e->usmUserNameLength;
        res->usmUserSecurityName =
            v3strcpy(e->usmUserSecurityName, e->usmUserSecurityNameLength);
        res->usmUserSecurityNameLength = e->usmUserSecurityNameLength;
        res->usmUserAuthProtocol       = e->usmUserAuthProtocol;
        res->usmUserAuthKey =
            v3strcpy(e->usmUserAuthKey, e->usmUserAuthKeyLength);
        res->usmUserAuthKeyLength = e->usmUserAuthKeyLength;
        res->usmUserPrivProtocol  = e->usmUserPrivProtocol;
        res->usmUserPrivKey =
            v3strcpy(e->usmUserPrivKey, e->usmUserPrivKeyLength);
    ...
    const SmiINT32 priv_proto, const OctetStr& priv_key)
{
    ...
    table[entries].usmUserEngineID =
        v3strcpy(engine_id.data(), engine_id.len());
    table[entries].usmUserNameLength = user_name.len();
    table[entries].usmUserName = v3strcpy(user_name.data(), user_name.len());
    table[entries].usmUserSecurityNameLength = sec_name.len();
    table[entries].usmUserSecurityName =
        v3strcpy(sec_name.data(), sec_name.len());
    table[entries].usmUserAuthProtocol  = auth_proto;
    table[entries].usmUserAuthKeyLength = auth_key.len();
    table[entries].usmUserAuthKey = v3strcpy(auth_key.data(), auth_key.len());
    table[entries].usmUserPrivProtocol  = priv_proto;
    table[entries].usmUserPrivKeyLength = priv_key.len();
    table[entries].usmUserPrivKey = v3strcpy(priv_key.data(), priv_key.len());
    entries++;
    return SNMPv3_USM_OK;
}

int USMUserTable::update_key(const OctetStr& user_name,
    const OctetStr& engine_id, const OctetStr& new_key, const int key_type)
{
    LOG_BEGIN(loggerModuleName, INFO_LOG | 7);
...
int USMUserTable::save_to_file(const char* name, AuthPriv* ap)
{
...

    {
        ...
        for (int i = 0; i < entries; ++i)
        {
            ...
            encodeString(table[i].usmUserEngineID,
                table[i].usmUserEngineIDLength, encoded);
            ...
            encodeString(
                table[i].usmUserName, table[i].usmUserNameLength, encoded);
            encoded[2 * table[i].usmUserNameLength] = '\n';
            if (fwrite(
                    encoded, 2 * table[i].usmUserNameLength + 1, 1, file_out)
                != 1)
            {
                failed = true;
                break;
            ...
            encodeString(table[i].usmUserAuthKey,
                table[i].usmUserAuthKeyLength, encoded);
            ...
            encodeString(table[i].usmUserPrivKey,
                table[i].usmUserPrivKeyLength, encoded);
...
int USMUserTable::load_from_file(const char* name, AuthPriv* ap)
{
    ...
    while (fgets((char*)line, MAX_LINE_LEN * 2, file_in))
    {
        ...
        LOG_BEGIN(loggerModuleName, INFO_LOG | 7);
        LOG("USMUserTable: Adding localized (user name) (eng id) (auth) "
            "(priv)");
        LOG(user_name.get_printable());
        LOG(engine_id.get_printable());
        LOG(auth_prot);
        ...
        if (add_entry(engine_id, user_name, user_security_name, auth_prot,
                auth_key, priv_prot, priv_key)
            == SNMPv3_USM_ERROR)
        {
            failed = true;
