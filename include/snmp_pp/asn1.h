...
// prototypes for decoding routines
DLLOPT unsigned char* asn_parse_int(
    unsigned char* data, int* datalength, unsigned char* type, SmiINT32* intp);

// FIXME: why mapped to SmiINT32 interface? CK
inline unsigned char* asn_parse_int(
...
DLLOPT unsigned char* asn_parse_unsigned_int(unsigned char* data,
    int* datalength, unsigned char* type, SmiUINT32* intp);

// FIXME: why mapped to SmiUINT32 interface? CK
inline unsigned char* asn_parse_unsigned_int(
    unsigned char* data, int* datalength, unsigned char* type, SmiINT32* intp)
{
    return asn_parse_unsigned_int(data, datalength, type, (SmiUINT32*)intp);
}

// prototypes for coding enroutines
...
inline unsigned char* asn_build_int(unsigned char* data, int* datalength,
    const unsigned char type, const SmiUINT32* intp)
{
    return asn_build_int(data, datalength, type, (const SmiINT32*)intp);
}

DLLOPT unsigned char* asn_build_unsigned_int(
    unsigned char* data, int* datalength, unsigned char type, SmiUINT32* intp);
...
DLLOPT unsigned char* asn_build_string(unsigned char* data, int* datalength,
    const unsigned char type, const unsigned char* string,
    const int strlength);

DLLOPT unsigned char* asn_parse_header(
    unsigned char* data, int* datalength, unsigned char* type);

DLLOPT unsigned char* asn_build_header(
...
DLLOPT unsigned char* asn_build_null(
    unsigned char* data, int* datalength, unsigned char type);
...
DLLOPT unsigned char* asn_parse_unsigned_int64(unsigned char* data,
    int* datalength, unsigned char* type, struct counter64* cp);
...
DLLOPT int snmp_build(struct snmp_pdu* pdu, unsigned char* packet,
    int* out_length, const snmp_version version,
...
DLLOPT unsigned char* build_vb(
    struct snmp_pdu* pdu, unsigned char* buf, int* buf_len);
...
DLLOPT int snmp_parse_data_pdu(
    struct snmp_pdu* pdu, unsigned char*& data, int& length);

DLLOPT int snmp_parse_vb(
    struct snmp_pdu* pdu, unsigned char*& data, int& data_len);
...
DLLOPT unsigned char* asn1_build_header_data(unsigned char* outBuf,
    ...
    unsigned char msgFlags, SmiINT32 securityModel);
