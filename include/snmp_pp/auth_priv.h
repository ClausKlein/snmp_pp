...
public:
     ...
     * @return SNMPv3_USM_OK on success
     */
    virtual int hash(const unsigned char* data, const unsigned int data_len,
     ...
     */
    virtual int auth_out_msg(const unsigned char* key, unsigned char* msg,
        const int msg_len, unsigned char* auth_par_ptr) = 0;
     ...
     *         SNMPv3_USM_ERROR for unexpected errors.
     */
    virtual int auth_inc_msg(const unsigned char* key, unsigned char* msg,
...
public:
     ...
     * @return SNMPv3_USM_OK on success
     */
    virtual int encrypt(const unsigned char* key, const unsigned int key_len,
     ...
     * @return SNMPv3_USM_OK on success
     */
    virtual int decrypt(const unsigned char* key, const unsigned int key_len,
        const unsigned char* buffer, const unsigned int buffer_len,
        unsigned char* out_buffer, unsigned int* out_buffer_len,
    ...
    virtual int extend_short_key(const unsigned char* password,
        const unsigned int password_len, const unsigned char* engine_id,
        const unsigned int engine_id_len, unsigned char* key,
        unsigned int* key_len, const unsigned int max_key_len, Auth* auth) = 0;
...
public:
    ...
    /**
     * Destructor, deletes all auth and priv protocol objects.
     */
    ~AuthPriv();
     ...
     * protocol.
     */
    int password_to_key_priv(const int auth_prot, const int priv_prot,
        const unsigned char* password, const unsigned int password_len,
        const unsigned char* engine_id, const unsigned int engine_id_len,
        unsigned char* key, unsigned int* key_len);
     ...
     */
    int get_keychange_value(const int auth_prot, const OctetStr& old_key,
     ...
     * Encrypt a message.
     */
    int encrypt_msg(const int priv_prot, const unsigned char* key,
     ...
     * Decrypt a message.
     */
    int decrypt_msg(const int priv_prot, const unsigned char* key,
        const unsigned int key_len, const unsigned char* buffer,
        const unsigned int buffer_len, unsigned char* out_buffer,
     ...
     */
    int auth_out_msg(const int auth_prot, const unsigned char* key,
        unsigned char* msg, const int msg_len, unsigned char* auth_par_ptr);

    /**
     * Check the authentication field of an incoming message
     */
    int auth_inc_msg(const int auth_prot, const unsigned char* key,
        unsigned char* msg, const int msg_len, unsigned char* auth_par_ptr,
...
public:
#    endif
    int encrypt(const unsigned char* key, const unsigned int key_len,
        const unsigned char* buffer, const unsigned int buffer_len,
        unsigned char* out_buffer, unsigned int* out_buffer_len,
        unsigned char* privacy_params, unsigned int* privacy_params_len,
