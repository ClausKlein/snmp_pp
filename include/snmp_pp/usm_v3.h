...
#    define SNMP_PRIVPROTOCOL_AES128W3DESKEYEXT \
        22 ///< AES128 with Key extension algorithm from 3DESEDE (non standard)
#    define SNMP_PRIVPROTOCOL_AES192W3DESKEYEXT \
        23 ///< AES192 with Key extension algorithm from 3DESEDE (non standard)
#    define SNMP_PRIVPROTOCOL_AES256W3DESKEYEXT \
        24 ///< AES256 with Key extension algorithm from 3DESEDE (non standard)
//@}
...
typedef void (*usm_add_user_callback)(const OctetStr& engine_id,
    const OctetStr& usm_user_name, const OctetStr& usm_user_security_name,
    const int auth_protocol, const OctetStr& auth_key, const int priv_protocol,
...
public:
     ...
     */
    USM(unsigned int engine_boots, const OctetStr& engine_id,
        const v3MP* v3_mp, unsigned int* msg_id, int& result);
     ...
     *           SNMP_v3_USM_ERROR (memory error, not initialized)
     */
    int add_usm_user(const OctetStr& user_name, const OctetStr& security_name,
        const SmiINT32 auth_protocol, const SmiINT32 priv_protocol,
        const OctetStr& auth_password, const OctetStr& priv_password);
     ...
     */
    int build_localized_keys(const OctetStr& engine_id, const int auth_prot,
    ...
    struct UsmKeyUpdate* key_update_prepare(const OctetStr& securityName,
        ...
        int& status, const OctetStr& oldpass = "",
        const OctetStr& oldengid = "", const OctetStr& newengid = "");
     ...
     *       for thread safety.
     */
    const UsmUserNameTableEntry* peek_next_user(
     ...
     *           of range
     */
    const struct UsmUserNameTableEntry* get_user(
...
private:
     ...
     * @param maxLength          - before call: length of the buffer
     *                             after call: bytes left in the buffer
     * @param globalData         - Buffer that contains the serialized
     * globalData
     * @param globalDataLength   - The length of this buffer
