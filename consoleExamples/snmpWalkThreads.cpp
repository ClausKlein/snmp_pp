...
void* runable(void* data)
{
    ...
    std::cout << "(" << t << "): "
              << "SNMP++ snmpWalk to " << address[t].get_printable()
              << " SNMPV"
#ifdef _SNMPv3
        ...
        == SNMP_CLASS_SUCCESS)
    {
        ...
        for (int z = 0; z < pdu.get_vb_count(); z++)
        {
            ...
            if (pdu.get_type() == REPORT_MSG)
            {
                ...
                vb.get_oid(tmp);
                std::cout << "(" << t << "): "
                          << "Received a reportPdu: " << snmp->error_msg(tmp)
                          << std::endl
            ...
            {
                std::cout << "(" << t << "): " << vb.get_printable_oid()
                          << " = ";
...
int main(int argc, char** argv)
{
    ...
    while (threads)
    {
        ...
        pthread_create(
            &thread[threads - 1], 0, &runable, (void*)new int(threads - 1));
#else
        int n = threads - 1;
        runable(&n);
#endif
